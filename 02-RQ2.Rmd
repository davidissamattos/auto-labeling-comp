...
d %>% 
  ...
  kable(
        ...
        digits = 3,
        linesep = "") %>% 
  pack_rows("10% missing labels",1, n_models) %>%
  pack_rows("50% missing labels",1+n_models,2*n_models) %>% 
...
d_acc_bt_50 <-  dplyr::tribble(~model0_name, ~model0, ~model1_name, ~model1, ~y, ~iterations, ~Dataset, ~DatasetType)
```
...
m50_fit <- stan(file = './models/rankingmodel.stan',
                     ...
                     warmup = 200,
                     iter = 2000)
saveRDS(m50_fit, file = "./data/m50_fit.RDS")
```
...
p<-ggplot(data=hpdi_algorithm, aes(x=Parameter))+
  geom_pointrange(aes(
    ...
    ymax=HPDI.higher, 
    y=Mean))+
  labs(y="Estimate", x="Model", title = "HPDI of the strength of the model with 50% missing labels")+
  coord_flip()
p
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms with 50% missing lables', 
      booktabs=T,
      label='ranking10',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('./paper/ranking50')
```
